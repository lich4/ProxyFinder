...
void ProxyValidateThread::run() {
    ...
    if (response.indexOf(proxycls->validate_key) != -1) {
        ...
        // 更新视图
        int l = 0;
        for (l = 0; l < proxycls->max_runlist.length(); l++) {
            QString proxy = QString("%1:%2").arg(proxycls->max_runlist[l].hostname)
                    .arg(proxycls->max_runlist[l].port);
            proxycls->update_runlist(l, 0, proxy);
            proxycls->update_runlist(l, 1, ip);
            proxycls->update_runlist(l, 2, QString("%1").arg(proxycls->max_runlist[l].timeout));
        }
...
void ProxySearchThread::run() {
    ...
    proxycls->mlock.lock();
    proxycls->update_status(QString("搜索代理:进行 验证代理:进行 搜索%1中的代理数:%2")
                .arg(url).arg(result_count));
    proxycls->mlock.unlock();
